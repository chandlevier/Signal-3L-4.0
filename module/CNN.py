    ...
    def __init__(self, config):
        ...
        self.conv = nn.Sequential(
                        nn.Conv1d(
                            ...
                            padding=math.floor(config["kernel_size"]/2)
                                ), self.activation) # 原文声明activation统一使用ReLU


    def forward(self, x):
        """x shape=(batch_size, max_text_len, 148)"""
        embed_x = x # shape: (256, 70, 256)
        # embed_x shape=(batch_size, max_text_len,  embedding_dim=148)
        embed_x = embed_x.permute(0, 2, 1)  # shape: (256, 256, 70)
        ...
        out = self.conv(embed_x.float())    # shape: (256, 512, 70)

        # out shape=(batch_size, max_text_len, out_channel=40/20)
        out = out.permute(0, 2, 1)  # shape: (256, 70, 512)
        return out
